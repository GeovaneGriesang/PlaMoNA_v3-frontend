...
export const obterMedicao = async (periodo) => {
    try {
        const response = await axios.post('https://plamona-q3aj.onrender.com/medicoes', { periodo }); // Corrigi o endereço IP para 127.0.0.1
...
export const obterComparacao = async (periodo, comp1, comp2) => {
  try {
    const response = await axios.post('https://plamona-q3aj.onrender.com/comparacao', {"periodo": periodo, "comp_1": comp1, "comp_2": comp2});
    return response.data;
  } catch (error) {
...
export const obterDia = async (periodo, comp1) => {
  try {
    const response = await axios.post('https://plamona-q3aj.onrender.com/comparacao', {"periodo": periodo, "comp_1": comp1});
    return response.data;
  } catch (error) {
    console.error('Erro Ao Puxar do banco de dados:', error);
  }
